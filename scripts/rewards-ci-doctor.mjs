...
const required = [
  ...
  "vitest.rewards-critical.config.ts",
...
try {
  ...
  for (const name of migrations) {
    ...
    seen.set(ts, [...(seen.get(ts) ?? []), name]);
  }
  for (const [ts, names] of seen) {
    if (names.length > 1) errors.push(`Duplicate migration timestamp ${ts}: ${names.join(", ")}`);
  }
} catch {
  errors.push("Unable to inspect migrations");
}
if (errors.length) {
