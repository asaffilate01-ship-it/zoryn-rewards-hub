...
function TenantsPage() {
  ...
  const retryMutation = useMutation({
    ...
    onSuccess: (res) => {
      if (res?.ok) toast.success(`${t("Event verarbeitet")} (+${res.amount ?? 0})`);
      else toast.error(res?.reason ?? t("Verarbeitung fehlgeschlagen"));
      void qc.invalidateQueries({ queryKey: ["tenantOverview"] });
    },
    onError: (e: Error) => toast.error(e.message),
  });

  if (isLoading) return <p className="text-sm text-muted-foreground">{t("Lade Mandanten…")}</p>;
    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {t("Multi-Tenant-Architektur: Programme, Wallets und eingehende Partner-Events.")}
        </p>
      </header>

      <section className="grid gap-4 sm:grid-cols-2">
        {(data?.tenants ?? []).map((tenant) => (
          <article key={tenant.id} className="rounded-2xl border border-border/60 bg-card/60 p-5">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="font-display text-[15px] font-semibold">{tenant.name}</p>
                  <p className="text-xs text-muted-foreground">
                    /{tenant.slug} · {tenant.mode}
                  </p>
                </div>
              </div>
            ...
            <dl className="mt-4 grid grid-cols-4 gap-2 text-center">
              {[
                { k: "Programme", v: tenant.programmes },
                { k: "Merchants", v: tenant.merchants },
                { k: "Mitglieder", v: tenant.memberships },
                { k: "Wallets", v: tenant.wallets },
              ].map((stat) => (
                <div
                  key={stat.k}
                  className="rounded-xl border border-border/50 bg-background/40 py-2"
                >
                  <dt className="text-[10px] uppercase tracking-wider text-muted-foreground">
                    {t(stat.k)}
                  </dt>
                  <dd className="font-display text-base font-semibold">{stat.v}</dd>
                </div>
              ))}
            </dl>
            <p className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground">
              <Wallet className="size-3.5" /> {t("Verfügbar gesamt:")}{" "}
              <span className="font-medium text-foreground">
                {tenant.available.toLocaleString("de-DE")}
              </span>
            </p>
          </article>
        ))}
      ...
      <section className="rounded-2xl border border-border/60 bg-card/60">
        <div className="flex items-center gap-2 border-b border-border/60 px-5 py-3.5">
          ...
          <h2 className="font-display text-sm font-semibold">{t("Eingehende Events")}</h2>
        </div>
        <div className="divide-y divide-border/50">
          ...
          {(data?.events ?? []).length === 0 && (
            <p className="px-5 py-6 text-sm text-muted-foreground">
              {t("Noch keine Events empfangen. Partner senden an")}{" "}
